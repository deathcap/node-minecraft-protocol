...
Client.prototype.setSocket = function(socket) {
  ...
  self.socket.on('data', function(data) {
    ...
    while (true) {
      ...
      if (packet.id !== undefined) { // only have id if shouldParsePayload true
        // emit convenient packet name (unique across multiple states)
        var packetName = protocol.packetNames[self.state].toClient[packet.id];
...
Client.prototype.write = function(packetId, params) {
  if (Array.isArray(packetId)) {
    ...
    packetId = protocol.packetIDs[this.state].toServer[packetId];
  }
