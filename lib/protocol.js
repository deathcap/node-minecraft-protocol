...
var packetStates = {};
var packetNames = function() {
  var ret = {};

  for (var stateName in states) {
    var state = states[stateName];

    ret[state] = {toClient: [], toServer: []};

    ['toClient', 'toServer'].forEach(function(direction) {
      for (var name in packetIDs[state][direction]) {
        var id = packetIDs[state][direction][name];

        ret[state][direction][id] = name;
        packetStates[name] = state;
      }
    });
  }
...
function createPacketBuffer(packetId, status, params, isServer) {
  var length = 0;
  if (typeof packetId === 'string' && typeof status !== 'string' && !params) {
    // simplified two-argument usage, createPacketBuffer(name, params)
    params = status;
    status = packetStates[packetId];
  }
  if (typeof packetId === 'string') packetId = packetIDs[status][!isServer ? 'toServer' : 'toClient'][packetId];
  assert.notEqual(packetId, undefined);

  var packet = get(packetId, status, !isServer);
...
function parsePacket(buffer, status, isServer, shouldParsePayload) {
  if (status == null) status = states.PLAY;
  ...
  if (shouldParsePayload)
    return parsePayload(packetId, status, isServer, buffer, cursor, length, lengthField.size);
  else
    return { 
...
function parsePayload(packetId, status, isServer, buffer, cursor, length, payloadSize) {
  ...
  var packetInfo = get(packetId, status, isServer);
  if (packetInfo === null) {
    return {
